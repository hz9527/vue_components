...
笔者的初衷只是用于一个页面复用picker而每次使用又不希望销毁该组件，然后更换itemList，而不是校验后来更新defaultIndex甚至某个列表，因此提供了limitMethods属性来定义校验规则  
...
### 自定义模版
为了style间互相不干扰，在这个版本中我们将style设置为scoped，因此为了弥补不能传入class的缺陷，设计了三个插槽

slot名|作用|默认使用|备注
---|---|---|---
head|在使用popPicker时，我们希望在上方加一个头部（如确认取消）|无|该插槽放置在picker上方了（position absolute bottom 100%）
center|为了显示出当前值，我们会在中间有两条灰线来显示|高度与itemHeight相同的两条线|滑动区域zindex为100，因此不要超过这个值
bg|显示背景|灰——透明——灰渐变|类似center，层不可高于100，因此也不能用于遮盖非选中值
