...
<div class="picker-item" :style='{"height":height * length + "px","lineHeight": height + "px"}' >
  <div v-if='type !== "content"' class="list-con" @touchcancel='moveEnd'  @touchstart='moveStart' @touchmove='move' @touchend='moveEnd'>
...
export default {
  ...
  methods: {
    ...
    getIndex (d) { // 参数为滚动方向，向上为正方向－1 ＋1
      ...
      // console.log(d, Math.log2(d + 2) * 0.1, Math.log2(2 - d) * 0.1)
      i += (d > 0 ? -(Math.log2(d + 2) * 0.1) : (Math.log2(2 - d) * 0.1))
      i = Math.round(i)
      if (this.curIndex !== i) {
        this.curIndex = i
        // emit
      ...
      if (!this.timer && !this.scrollTimer) {
        this.revisedTimer = requestAnimationFrame(() => {
          ...
          if (this.preTop !== this.target.scrollTop) {
            ...
            console.log(this.target.scrollTop, this.preTop)
  ...
  mounted () {
    if (this.type !== 'content') {
      ...
      this.target.addEventListener('scroll', () => {
        !this.lock && !this.timer && this.watchScroll()
      })
    }
  },
  created () {
